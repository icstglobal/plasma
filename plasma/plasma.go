...
func New(config *Config) (*Plasma, error) {
	...
	utxoSet := core.NewUTXOSet(chainDb)
	log.WithField("utxoset", utxoSet).Debug("try to init tx poll")
	eth.txPool = core.NewTxPool(config.TxPool, eth.chainConfig, eth.blockchain, utxoSet)
